...
class Network():
    ...
    def run_for_dur(self, dur):
        ...
        for sender in self.senders:
            ...
            sender.loss_exp_avg = 0.875 * sender.loss_exp_avg + 0.125 * loss
            ...
            T = 10 * throughput / (8 * BYTES_PER_PACKET)
            X = 10 * sendrate / (8 * BYTES_PER_PACKET)
            F = 0.1 * X * abs((sendrate / sendrate_sum - 1 / len(self.senders)))
            #print([T * self.sigmoid(loss - 0.05),  -X * loss, -F])
            #if ((-X * loss) < -1000):
            #    print(sendrate)
            #    print()

            # reward = (T * self.sigmoid(loss - 0.05) - X * loss - F) * REWARD_SCALE
            reward = (T * self.sigmoid(sender.loss_exp_avg - 0.05) - X * sender.loss_exp_avg - F) * REWARD_SCALE
            #reward = (T * self.sigmoid(loss - 0.05) - X * loss) * REWARD_SCALE
