...
class Network():
    ...
    def run_for_dur(self, dur):
        ...
        for sender, penalty in zip(self.senders, penalties):
            ...
            P = 0.1 * X * abs((sender.rate / self.sendrate_sum - 1 / len(self.senders)))
            ...
            print([T * self.sigmoid(sender.loss - 0.05), - X * sender.loss, -P, sender.reward * 10])
    ...
    def _get_all_sender_obs(self):
        ...
        for sender in self.senders:
            ...
            # ! sender_obs.append(np.array(sender.get_obs()).reshape(-1,))
                if ((sender.rate - sender.prev_rate) / sender.prev_rate > 99.0):
                    print(sender.rate)
                    print(sender.prev_rate)
        return sender_obs
