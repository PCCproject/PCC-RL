...
def main():
    ...
    for link_dir in link_dirs:
        ...
        for cc in TARGET_CCS:
            ...
            for trace_file in tqdm(sorted(glob.glob(os.path.join(link_dir,"{}_datalink_run[1,3].log".format(cc))))):
                ...
                save_dir = os.path.join(args.log_dir, args.conn_type, link_name, os.path.splitext(os.path.basename(trace_file))[0])
                save_dirs.append(save_dir)

    bbr_old_rewards = load_cc_rewards_across_traces(
        traces, [os.path.join(save_dir, "bbr_old", "bbr_old_summary.csv") for save_dir in save_dirs])
    bbr_rewards = load_cc_rewards_across_traces(
        traces, [os.path.join(save_dir, "bbr", "bbr_summary.csv") for save_dir in save_dirs])
    cubic_rewards = load_cc_rewards_across_traces(
        traces, [os.path.join(save_dir, "cubic", "cubic_summary.csv") for save_dir in save_dirs])
    print(bbr_rewards)
    steps = []
    genet_bbr_rewards = []
    genet_cubic_rewards = []
    for bo in range(18):
        ...
        tmp_rewards = load_cc_rewards_across_traces(
            traces, [os.path.join(save_dir, 'genet_bbr', 'bo_{}'.format(bo), "seed_{}".format(genet_seed), 'step_64800', 'aurora_packet_log.csv') for save_dir in save_dirs])
            # traces, [os.path.join(save_dir, 'genet_bbr', 'bo_{}'.format(bo), 'step_64800', 'aurora_summary.csv') for save_dir in save_dirs])
        steps.append(bo * 72000)
        print(tmp_rewards)
        genet_bbr_rewards.append(np.mean(tmp_rewards))
        tmp_rewards = load_cc_rewards_across_traces(
            traces, [os.path.join(save_dir, 'genet_cubic', 'bo_{}'.format(bo), "seed_{}".format(genet_seed),'step_64800','aurora_packet_log.csv') for save_dir in save_dirs])
            # traces, [os.path.join(save_dir, 'genet_cubic', 'bo_{}'.format(bo), 'step_64800','aurora_summary.csv') for save_dir in save_dirs])
        print(save_dirs)
        print(tmp_rewards)
    ...
    udr2_reward_errs = []
    udr3_reward_errs = []
    udr_steps = [0, 43200, 100800, 158400, 216000, 259200, 316800, 374400, 432000, 48900]
