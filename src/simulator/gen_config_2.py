...
n_traces = 10
save_dir = '../../data/synthetic_traces_30s_0505'
...
for _ in range(10):
    DEFAULT_CONFIGS.append(
        { "bandwidth": round(10 ** np.random.uniform(np.log10(1), np.log10(6), 1).item(), 2),
          "delay": round(np.random.uniform(5, 200, 1).item(), 2),
          "loss": round(np.random.uniform(0, 0.0, 1).item(), 2),
          "queue": int(10 ** np.random.uniform(np.log10(2), np.log10(200), 1).item()),
          "T_s": round(np.random.uniform(0, 6, 1).item(), 2),
          "delay_noise": round(np.random.uniform(0, 100, 1).item(), 2)})


for bw in np.linspace(np.log10(1), np.log10(6), 6):
    for default_config in DEFAULT_CONFIGS:
        DELAY = 100
        QUEUE = 20
        bandwidth_range = [1, 10**bw]
        bw = round(10 ** bw), 2)
