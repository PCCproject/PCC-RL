...
#         model_path_before = os.path.join(ROOT, 'seed_{}'.format(seed), 'bo_{}'.format(bo-1), 'model_step_64800.ckpt')
    ...
    if not os.path.exists(model_path_after+'.index'):
...
#     config = [config]
    ...
    traces = generate_traces(config_file, 10, 30)
    ...
    test_on_traces(MODEL_PATH_BEFORE, traces, before_save_dirs, 8, seed, False, True)
    test_on_traces(model_path_after, traces, after_save_dirs, 10, seed, False, True)

    baseline = BBR_old(False)
    baseline.test_on_traces(traces, bbr_old_save_dirs, plot_flag=True, n_proc=8)
