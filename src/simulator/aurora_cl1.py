...
class SaveOnBestTrainingRewardCallback(BaseCallback):
    ...
    def _on_step(self) -> bool:
        if self.n_calls % self.check_freq == 0:
            i = int(self.n_calls // (self.check_freq * 20))
            self.model.env.config_file = self.aurora.config_files[i]
...
class Aurora():
    ...
    def train(self, config_files: List[str], total_timesteps: int, tot_trace_cnt: int,
        ...
        if not config_files and not training_traces:
            raise ValueError("Neither configuration file nor training_traces "
                             "are provided. Please provide one.")
        elif config_files and training_traces:
            raise ValueError("Both configuration file and training_traces are "
                             "provided. Please choose one.")
        elif config_files and not training_traces:
            training_traces = generate_traces(config_files[0], tot_trace_cnt,
                                              duration=30)
        # generate validation traces
        if not config_files and not validation_traces:
            raise ValueError("Neither configuration file nor validation_traces"
                             " are provided. Please provide one.")
        elif config_files and validation_traces:
            raise ValueError("Both configuration file and validation_traces "
                             "are provided. Please choose one.")
        elif config_files and not validation_traces:
            validation_traces = generate_traces( config_files[0], 20, duration=30)
        ...
        env = gym.make('PccNs-v0', traces=training_traces,
                       train_flag=True, delta_scale=self.delta_scale, config_file=config_files[0])
        env.seed(self.seed)
        ...
        callback = SaveOnBestTrainingRewardCallback(
            ...
            config_file=config_files[0], validation_flag=validation_flag)
