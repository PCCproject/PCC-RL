...
from simulator.network_simulator.pcc.monitor_interval import MonitorInterval, \
        PacketRttSample, LostPacketSample
from simulator.network_simulator.pcc.vivace import vivace_latency
...
class MonitorIntervalQueue:
    ...
    def __init__(self, sender: "vivace_latency.VivaceLatencySender") -> None:
        self.q = []
        self.sender = sender
    ...
    def on_packet_sent(self, pkt: Packet, sent_interval: float) -> None:
        if self.empty():
            print("MI queue is empty!", sys.stderr)
            return
        cur_mi = self.q[-1]
    ...
    def on_packet_acked(self, pkt, ack_interval: float, latest_rtt: float,
        ...
        for mi in self.q:
            ...
            for pending_acked_pkt in self.pending_acked_packets:
                if mi.contain_pkt(pending_acked_pkt.pkt_id):
                    if mi.bytes_acked == 0:
                    ...
                    mi.packet_rtt_samples.append(PacketRttSample(
                        pending_acked_pkt.pkt_id, self.pending_rtt, self.pending_event_time,
    ...
    def on_packet_lost(self, pkt, avg_rtt: float, min_rtt: float) -> None:
        ...
        for mi in self.q:
            ...
            if self.is_utility_available(mi):
                ...
                self.num_available_intervals += 1
                continue
            if mi.contain_pkt(pkt.pkt_id):
                mi.bytes_lost += pkt.pkt_size
                mi.lost_packet_samples.append(LostPacketSample(
    ...
    def is_utility_available(self, interval: MonitorInterval):
        return (interval.has_enough_reliable_rtt and
          interval.bytes_acked + interval.bytes_lost == interval.bytes_sent)


    def has_invalid_utility(self, interval: MonitorInterval):
