...
# for bw in np.linspace(np.log(0.6), np.log(5), 10):
#     DELAY = 100
#     QUEUE = 20
#     bandwidth_range = [np.exp(bw), np.exp(bw)]
#     bw = round(np.exp(bw), 2)
...
#
# for t_s in np.arange(0, 7, 0.5):
#     bandwidth_range = [0.6, 5]
#     delay_range = [DELAY, DELAY]
#     d_bw = 0.2
#     d_delay = 0
#     T_s = t_s
...
#
#
# # for delay_noise in np.concatenate([np.arange(0, 0.1, 0.01), np.arange(0.1, 1.1, 0.1)])*100:
# #     bandwidth_range = [0.6, 0.6]
# #     delay_range = [5, 5]
# #     d_bw = 0
# #     d_delay = 0
